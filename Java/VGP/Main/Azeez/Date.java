...
public class Date{
	...
	private boolean dateOK(int monthInt, int dayInt, int yearInt)
	{
		return ( (monthInt >= 1) && (monthInt <= 12) &&
				(dayInt >= 1) && (dayInt <= finalDate(monthInt)) &&
	...
	private boolean dateOK(String monthString, int dayInt, int yearInt)
	{
		return ( monthOK(monthString) &&
				(dayInt >= 1) && (dayInt <= finalDate(monthString)) &&
				(yearInt >= 1000) && (yearInt <= 9999) );
	}
	...
	private int finalDate(int monthNumber)
	{
		switch (monthNumber)
		{
		case 1:
	...
	private int finalDate(String month2){
	...
	public void advanceDate(){
		...
		if(day> finalDate()){
			if(monthInt() + 1 >= 13){
				month = this.monthString(1);
				year++;
			}else{ 
				month = this.monthString(monthInt() + 1);
			}
		}
	}
}
