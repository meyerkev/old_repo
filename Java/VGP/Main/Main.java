...
//		}
	...
	public void updateGame(){
		gameTime = time - System.currentTimeMillis()+ startTime;
		if(gameTime< 0){
			lose();
		}
...
//		}
	...
	public void paint(Graphics g){
		...
		if (!lost){
			...
			if(gameTime > 0){
				g.drawString(Double.toString(Double.parseDouble(Long.toString(gameTime))/1000), getSize().width - 55, 10);
			}
		}
		if(p.hasWon()){
			paused = true;
			...
			g.setColor(Color.green);
			g.drawString("You Win with a score of " + score, (getSize().width/2) - 40, (getSize().height/2)- 10);
